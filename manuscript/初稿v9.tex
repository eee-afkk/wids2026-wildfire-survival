...
%=================================================================
\abstract{%
Timely wildfire threat-time estimates could support pre-evacuation readiness, yet existing approaches rarely address censoring, multi-horizon coherence, and calibration in one pipeline.
We developed a survival--probability fusion prototype combining an XGBoost accelerated failure time backbone, horizon-specific heads, post-hoc calibration, and monotone fusion to generate coherent 12, 24, and 48~h threat probabilities.
Using 221 wildfire--zone encounters from the WiDS Datathon 2026 (69 events, 152 censored), we evaluated the framework with nested five-fold cross-validation and inner cross-fitting.
The full model achieved a C-index proxy of 0.940, Uno's IPCW C-index of 0.941 (95\% bootstrap CI 0.924--0.959), and a mean Brier score of 0.041, with zero post-fusion monotonicity violations.
At 24~h, observed event rates across predicted risk terciles were 0.0\%, 4.5\%, and 92.3\%.
Simpler comparators matched or exceeded the full architecture on some aggregate metrics, while auxiliary robustness checks suggested only modest grouped-validation optimism but greater censoring sensitivity at 48~h.
The framework is therefore a methodological prototype for censor-aware multi-horizon wildfire threat forecasting; formal incident-grouped and external spatiotemporal validation remain necessary before practical use.
}
...
The baseline comparison indicates that the full prototype achieved competitive discrimination under internal validation and substantially outperformed the physics-informed ETA-only baseline ($\Delta$C-index = +0.032), but it did not uniformly dominate the stronger alternatives.
At the present sample size, the framework's value therefore lies primarily in its coherent censor-aware multi-horizon structure rather than in aggregate metric superiority.
Table~\ref{tab:baseline} organises the models into three functional tiers to clarify this interpretation.
...
The full model numerically outperformed the AFT-only baseline on discrimination (C-index: 0.940 vs.\ 0.938), though the IBS was slightly higher for the full model (0.041 vs.\ 0.039), consistent with the interpretation that additional stacking modules introduce estimation variance at low EPV.
...
These results reinforce that the framework's present contribution is structural: censor-aware formulation, monotone multi-horizon coherence, and modular extensibility that the per-horizon XGBoost does not provide by construction.
The comparison with RSF is more nuanced because RSF is itself a censor-aware survival model with inherently monotone CDF-derived probabilities.
At this data scale, RSF's superior IBS and greater seed stability suggest that a single unified survival model may absorb the available signal more efficiently than the full stacked architecture, while the framework's modular fusion design remains a research-oriented advantage whose practical benefits still require confirmation on larger datasets.
...
Because verified wildfire-incident identifiers were unavailable, auxiliary robustness checks were conducted with a simplified lean AFT model to assess whether the primary internal-validation results were materially inflated by split design.
Under proxy incident-grouped cross-validation, the mean C-index decreased only modestly from 0.9318 $\pm$ 0.0119 under standard stratified CV to 0.9283 $\pm$ 0.0323 under grouped CV, while the mean Brier score increased from 0.1352 $\pm$ 0.0341 to 0.1541 $\pm$ 0.0761 (Supplementary Table~S8).
The limited average C-index drop suggests that major leakage-driven optimism is unlikely at the present data scale, but the grouped split produced both worse mean Brier score and substantially wider between-fold variability, indicating appreciably greater uncertainty once approximate incident structure is enforced.
A complementary leave-one-month-out temporal blocked analysis yielded an $N$-weighted mean C-index of 0.935 across the six months with at least 10 eligible observations.
The three largest holdout months (Jun--Aug; $n = 33$--70) remained within a relatively narrow range ($C = 0.923$--0.943), whereas the smaller March, May, and September holdouts were more variable, including a likely artefactual March estimate of $C = 1.000$.
These temporal results are therefore supportive rather than definitive evidence against strong time-based overfitting.

The second robustness question concerns the independent right-censoring assumption.
Worst-case/best-case event-rate bounds showed sensitivity ranges of 0.027 at 12~h, 0.113 at 24~h, and 0.249 at 48~h, with the 48~h base event rate of 0.398 spanning a plausible range from 0.299 to 0.548 under extreme censoring scenarios (Supplementary Table~S9).
Thus, short-horizon event-rate estimates appear relatively stable, whereas the 48~h outputs are materially more assumption-sensitive and should be interpreted with greater caution than the 24~h horizon.
Figure~10 summarises the grouped-validation discrimination and mean-Brier comparisons together with censoring sensitivity; standalone full-page versions are retained as Supplementary Figures~S2 and~S4.
...
Based on internal validation only, the 24~h horizon (slope~=~0.998) is the most promising for eventual operational use, with near-ideal calibration; the 48~h horizon (slope~=~1.075) may still be useful for longer-lead triage but is more assumption-sensitive because censoring sensitivity is largest at this horizon, as shown in the robustness results above; and the 12~h horizon (slope~=~0.805) exhibits moderate overconfidence that restricts its use to relative ranking or requires post-fusion recalibration before threshold-based triggers.
...
The auxiliary robustness checks reported in the Results section are supportive but not definitive.
The proxy incident-grouped analysis showed only a modest mean C-index decrease relative to standard stratified CV, suggesting that major leakage-driven inflation is unlikely, yet the grouped split also worsened mean Brier score and produced substantially wider between-fold variability, indicating greater uncertainty once approximate incident structure is enforced.
Temporal blocking produced similar discrimination in the three largest fire-season holdout months, but the smaller March, May, and September subsets remained unstable.
Most importantly, informative-censoring sensitivity increased monotonically with horizon length, and the 48~h event-rate estimate spanned 29.9\%--54.8\% under worst/best-case assumptions.
This strengthens the interpretation that 24~h is the most defensible operational horizon at present, whereas 48~h probabilities are informative but more assumption-sensitive.
...
Second, the outer cross-validation folds were stratified by event-time strata but not grouped by verified wildfire incident identifier.
...
The auxiliary proxy grouped-CV analysis reported above suggested only modest average optimism ($\Delta$C-index = $-0.0035$), but grouped splitting also worsened the mean Brier score and materially widened fold-to-fold variability; because the groups were constructed from feature similarity rather than ground-truth incident IDs, formal incident-level grouped validation remains necessary when verified identifiers become available.
...
As shown in the robustness results above, worst/best-case event-rate bounds widened from 0.027 at 12~h to 0.249 at 48~h, indicating that longer-horizon probabilities are materially more assumption-sensitive than the 24~h horizon.
Future work should therefore explore censoring-robust methods such as sensitivity-weighted survival models or explicit competing-risk formulations that treat containment as a distinct event type.
...
Auxiliary robustness analyses using a lean AFT model suggested only modest grouped-validation optimism and similar discrimination under temporal blocking in the larger fire-season months, but informative-censoring sensitivity increased materially at 48~h.
Accordingly, the 24~h horizon currently appears the most defensible operational target, whereas 48~h probabilities may still be useful for longer-lead triage but should be interpreted more cautiously.
