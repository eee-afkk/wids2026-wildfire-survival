...
%=================================================================
\abstract{%
Evacuation triage during wildfires depends on knowing not just whether a fire will arrive, but when. Existing prediction approaches rarely handle right censoring, cross-horizon probability coherence, and calibration together.
We developed a survival--probability fusion prototype that combines an XGBoost accelerated failure time backbone with horizon-specific probability heads, beta calibration, and monotone fusion to produce coherent 12, 24, and 48~h threat probabilities for wildfire--zone encounters.
Using 221 observations (69 events, 152 censored) from the WiDS Datathon 2026, we evaluated the framework under nested five-fold cross-validation with inner cross-fitting. All results reflect internal validation only; no external spatiotemporal or incident-grouped validation was performed.
The model achieved Uno's IPCW C-index of 0.941 (95\% bootstrap CI 0.924--0.959) and mean Brier score of 0.041. The 24~h horizon was best calibrated (slope = 0.998) and separated risk terciles sharply (observed event rates: 0.0\%, 4.5\%, 92.3\%). Monotone fusion corrected incoherent probability sequences in 24.9\% of observations at the 24--48~h transition.
At the present sample size (events-per-variable $\approx$ 2.0), a Random Survival Forest achieved lower probability error and greater stability, and lean AFT variants matched the full architecture's discrimination. These findings suggest that simpler censor-aware models may be preferable in practice at low EPV, while the full framework serves as a methodological prototype whose added complexity requires larger datasets to justify.
}
...
Horizon-specific threat probabilities directly inform pre-evacuation readiness. Emergency managers need to rank simultaneously threatened zones by time-criticality, stage resources ahead of a fire front, and trigger zone-level alerts with enough lead time~\cite{Finney2001,Xi2019,Stephens2018}.
With the frequency, intensity, and spatial extent of large wildfires rising over the past two decades~\cite{Jain2022,Cunningham2024,Turco2023,Luo2024,Jones2022,Kolden2024,Higuera2021}, so has the demand for calibrated, multi-horizon decision-support tools that go beyond distance-based heuristics~\cite{Radeloff2023,Tonini2020}.
...
Third, survival analysis approaches---such as Cox proportional-hazards or accelerated failure time (AFT) models---handle censoring natively and produce survival curves from which horizon-specific cumulative incidence probabilities $P(T \leq h)$ can be derived. These probabilities are monotone non-decreasing by construction~\cite{Wiegrebe2024,Taylor2013}.
When \emph{separate} horizon-specific classifiers are trained independently, however, the resulting probability estimates are not guaranteed to be consistent across horizons. The survival-derived probabilities may also need post-hoc calibration to align with observed event rates at each discrete horizon~\cite{VanCalster2019}.

Several gaps in current methods limit their use for operational evacuation support.
...
Third, discrimination alone is insufficient for threshold-based evacuation triggers; absolute probability calibration is needed if zone-level decision rules are to work in practice~\cite{VanCalster2019}.

In this study, we develop a survival--probability fusion framework to address these gaps.
The framework is evaluated as an internally validated prototype on 221 wildfire--zone encounters (69 events). With so few events relative to the number of features, we do not expect the full multi-module architecture to outperform simpler alternatives on aggregate metrics; the goal is instead to show how these components can be integrated and to clarify when simpler models work just as well.
We make three contributions:
\begin{enumerate}
    \item A censor-aware prediction pipeline that unifies an AFT survival backbone with horizon-specific probability heads, beta calibration, and monotone fusion to produce coherent 12, 24, and 48~h threat probabilities. The three horizons serve different operational roles: 24~h is best calibrated and most relevant for pre-evacuation staging; 12~h is useful for relative triage but harder to calibrate; and 48~h is informative but sensitive to censoring assumptions. Monotone enforcement corrects incoherent probability sequences in about 25\% of observations at the 24--48~h transition;
    ...
    \item A head-to-head comparison showing that, at the present low events-per-variable ratio, a Random Survival Forest and lean AFT-centred variants match or exceed the full architecture on aggregate metrics---an outcome that is itself informative for practitioners choosing models under data constraints.
\end{enumerate}
...
Censoring from fire containment may not be fully non-informative: containment is an intervention correlated with fire behaviour covariates rather than a random process, departing from the standard independent right-censoring assumption~\cite{Steyerberg2019}.
...
These thresholds are pragmatic operational cutoffs based on general fire-management considerations. The 4500~m value falls within the range at which ember transport and spot-fire ignition have been reported under moderate-to-high wind conditions in coniferous fuel types~\cite{Cruz2003,Finney2001}, while 7000~m is broadly consistent with outer perimeters used for pre-evacuation staging in California~\cite{Dolan2022}. Both values are engineering heuristics rather than established domain consensus, and their appropriateness may vary across fuel types and terrain.
...
The value $\lambda = 1.0$ was chosen to impose moderate regularisation: it balances the data-driven Brier loss (whose magnitude is $O(10^{-2})$ per observation) against a meaningful pull toward equal weighting, which discourages dominance by any single base learner without suppressing informative weight differentiation.
...
The present dataset does not provide a verified incident identifier, so a grouped split by fire event was not possible.
...
Because these auxiliary analyses were designed to probe validation bias under more restrictive splits, rather than to re-estimate the full prototype under severely data-limited partitions, they are better understood as \emph{relative-bias diagnostics} on the validation scheme than as absolute performance estimates for the full multi-module architecture.
...
Given the modest sample size, effect sizes and 95\% bootstrap confidence intervals are the primary inferential focus; $p$-values serve as directional indicators and are not intended as formal hypothesis tests.
...
This summary measure is used as a proxy for the continuous-time integrated Brier score~\cite{Graf1999}; it is not a true continuous-time integration, and we note this distinction.
...
The full prototype clearly outperformed the physics-informed ETA-only baseline ($\Delta$C-index = +0.032) but did not uniformly dominate the stronger alternatives.
At this sample size, the framework's value lies in its coherent censor-aware structure rather than in beating all baselines on every metric.
...
Among the \emph{external comparators}, per-horizon XGBoost achieved the highest C-index (0.942) with comparable IBS (0.040), showing that a well-tuned single-model approach can match the full framework at this sample size.
...
The comparison with RSF is worth noting separately: RSF is itself censor-aware with naturally monotone probabilities.
At this data scale, RSF's superior IBS and greater seed stability suggest that a single unified survival model absorbs the available signal more efficiently than the full stacked architecture. The framework's modular design remains a research-oriented advantage that needs confirmation on larger datasets.
...
The complete framework showed a numerically favourable discrimination profile in the ablation study, though external baselines such as per-horizon XGBoost and RSF achieved comparable or better aggregate metrics (see Section~\ref{sec:baseline_results}).
The modules contributed unevenly.
...
The build-up analysis showed that AFT~+~Direct had a significant deficit relative to the full model ($\Delta$C-index = +0.002, $p = 0.024$), while AFT-only showed a comparable but non-significant gap ($\Delta = +0.002$, $p = 0.340$).
This suggests that direct heads on their own introduce estimation variance that the other modules (Cox, gate prior, IPCW, simple distance) offset in the full configuration. Removing direct heads from the full model marginally improved IBS ($\Delta = -0.002$, $p = 0.126$), which is consistent with this reading.
...
Without monotone enforcement, approximately one-quarter of observations would have received logically incoherent probability sequences at the 24--48~h transition, confirming that the enforcement module does real corrective work rather than serving as a formality.
...
The limited C-index drop suggests that major leakage-driven optimism is unlikely, but the grouped split produced worse probability error and much wider fold-to-fold variability.
...
Short-horizon estimates are relatively stable, whereas 48~h outputs depend more heavily on the censoring assumption.
...
This study asked whether censor-aware survival modelling, multi-horizon calibration, and monotone enforcement can be usefully combined for wildfire threat-time forecasting. The answer is yes in principle, but with a caveat: at the present sample size (221 observations, 69 events, EPV~$\approx$~2.0), and with only internal validation available, the added complexity of the full multi-module architecture did not translate into better metrics. RSF achieved lower probability error, per-horizon XGBoost slightly higher discrimination, and lean AFT variants matched the full model's performance with far fewer moving parts.

This does not make the framework uninformative. It achieved strong discrimination (Uno-C = 0.941), low Brier scores (0.027--0.060), and sharp risk stratification at 24~h (tercile event rates: 0.0\%, 4.5\%, 92.3\%). The 24~h horizon proved best calibrated (slope = 0.998), the 12~h horizon was harder to calibrate (slope = 0.805) because it covers a short and highly variable window, and the 48~h horizon was slightly conservative (slope = 1.075) and more sensitive to assumptions about informative censoring. These horizon-level differences have direct operational meaning: the 24~h output is the most trustworthy for absolute threshold-based evacuation triggers, the 12~h output is better suited for relative zone ranking, and the 48~h output should be interpreted with extra caution.

What the study does contribute is two things. First, it shows a principled way to produce coherent multi-horizon threat probabilities under censoring. Second, the evaluation provides a fair comparison showing that simpler censor-aware models---particularly RSF and lean AFT variants---are equally effective at this data scale. For practitioners working with limited wildfire encounter data, the second point may be the more useful one.

\paragraph{Three-tier model roles.}
The models fall into three functional tiers.
The \emph{full model} is a methodological prototype that integrates censor-aware survival modelling, horizon-targeted heads, monotone enforcement, and modular ablation.
The \emph{lean AFT variants} (AFT~+~Cox, AFT~+~Cox~+~Simple) are practical alternatives that preserve censor-awareness and monotone coherence with much simpler architectures.
...
RSF is itself censor-aware with naturally monotone probabilities; the framework differs from RSF in its modular fusion architecture and horizon-targeted calibration pipeline.

\paragraph{Framework design and ablation insights.}
Existing wildfire threat models have focused on spatial susceptibility mapping at a single horizon~\cite{Jaafari2019,Gigovic2019,Chen2024wui,Kaur2022} or physics-based fire-spread simulation~\cite{Finney2001,Cruz2003,Shu2020,Pais2021,Zheng2021}. Neither approach produces calibrated multi-horizon probabilities under censoring.
Evacuation decision support has been studied through vulnerability assessments and trigger-point analyses~\cite{Dolan2022,Paveglio2019,Alcasena2019}, and time-to-event perspectives appear in the broader wildfire risk literature~\cite{Xi2019,Taylor2013}. Zone-level threat prediction with calibrated multi-horizon output under censoring, however, has not been addressed before.
...
The ablation results further show that direct heads need the other modules to function well. AFT~+~Direct showed a significant C-index deficit ($p = 0.024$), while AFT-only showed a non-significant deficit ($p = 0.340$). This means that direct heads on their own introduce variance that the remaining modules offset.
This pattern suggests that the modules work together rather than contributing independently.

\paragraph{Module prioritisation under low EPV.}
Combining evidence from the ablation, lean-variant, and stability analyses, the most practical reduced architecture at EPV~$\approx$~2.0 centres on the AFT backbone with optional Cox (for discrimination) and simple distance (for probability anchoring).
...
\paragraph{Calibration and what each horizon is good for.}
The calibration metrics in Table~\ref{tab:calibration} refer to the final fused probabilities.
The 12~h horizon is the hardest to calibrate because it covers a short window where fire behaviour is highly variable---wind shifts, terrain channelling, and spot-fire ignition can all change the trajectory within hours. Its slope of 0.805 means the model is overconfident at 12~h, so threshold-based evacuation triggers at this horizon need recalibration or should be used only for relative zone ranking.
The 24~h slope of 0.998 makes it the best horizon for absolute probability-based decisions, such as initiating pre-evacuation preparation when $P(T \leq 24) > 0.3$.
The 48~h slope of 1.075 indicates slight conservatism, compounded by the fact that censoring sensitivity is largest at this horizon (sensitivity range = 0.249). For a fire manager, this means 48~h outputs are useful for longer-lead resource staging but should not be treated as precise.
A structural factor also contributes to the 12~h pattern: beta calibration targets each base head independently, but stacking and monotone fusion reshape the distribution afterwards. Joint post-fusion temperature scaling corrected the 12~h slope to 0.964 without degrading other horizons (Table~\ref{tab:recalibration}).
...
The best practical choices are lean AFT variants when modularity is desired, or RSF when predictive robustness is the priority.
...
Informative-censoring sensitivity grew with horizon length: the 48~h event-rate estimate spanned 29.9\%--54.8\% under extreme assumptions, which further supports 24~h as the most reliable operational horizon.

\paragraph{Practical implications.}
The risk stratification capability may be the most useful contribution in practice: a wildfire--zone pair at 3200~m with closing speed 400~m/h and growth rate 50~ha/h would yield fused probabilities of approximately $P(T \leq 12) = 0.72$, $P(T \leq 24) = 0.91$, $P(T \leq 48) = 0.95$, placing it in the high-risk tercile.
...
The framework is a decision-support tool, not an autonomous system; deployment would require integration with near-real-time fire-behaviour feeds~\cite{Radeloff2023} and evaluation against agency-specific thresholds.

\paragraph{Strengths and limitations.}
The main strengths of this study are the censor-aware problem formulation, the nested cross-validation with inner cross-fitting to prevent leakage, and the enforcement of multi-horizon monotonicity. The ablation and baseline analyses are more thorough than is typical for wildfire prediction studies, and the lean-variant evaluation and five-seed stability analysis provide secondary checks. We also report openly that simpler alternatives matched or exceeded the full framework, which we think is more useful to practitioners than only showing favourable results.
Deeper comparisons against deep survival models (e.g., DeepSurv~\cite{Katzman2018}) and discrete-time hazard models would strengthen future evaluations.

The study has several clear limitations.
First, the sample size ($n = 221$, 69 events, EPV~$\approx$~2.0) is the most important constraint: it is insufficient to reliably support the full modelling complexity, and is the simplest explanation for why the full model did not outperform simpler competitors~\cite{Riley2020}.
...
We developed and internally validated a survival--probability fusion framework for multi-horizon wildfire threat forecasting under right censoring. All results are based on internal cross-validation of 221 wildfire--zone encounters (69 events); no external or incident-grouped validation has been performed, so the findings should be read as a proof of concept rather than evidence of deployment readiness.

Within this scope, the framework produced coherent, monotonic probability estimates at 12, 24, and 48~h with strong discrimination (Uno-C = 0.941) and low probability error (Brier 0.027--0.060). The 24~h horizon was best calibrated (slope = 0.998) and most suitable for absolute threshold-based evacuation triggers. The 12~h horizon is useful for relative zone ranking but less reliable for fixed thresholds (slope = 0.805). The 48~h horizon was slightly conservative and more sensitive to censoring assumptions, so its outputs should be treated with extra caution. Monotone enforcement corrected incoherent probability sequences in about 25\% of observations, confirming that this step is needed in stacked multi-horizon setups.

An equally important finding is that simpler models performed well. RSF achieved lower probability error and greater seed-to-seed stability, and lean AFT variants matched the full model's discrimination with far simpler architectures. At the present data scale (EPV~$\approx$~2.0), a lean AFT pipeline with monotone fusion---or an RSF---is the more practical choice. The full architecture is best understood as a research prototype; whether its added modules pay off is a question for larger datasets.

Future work should focus on incident-level grouped validation with verified identifiers, external spatiotemporal validation across different fire regimes, and comparison with discrete-time hazard and deep survival models~\cite{Katzman2018}. Testing in operational early warning settings with real-time fire-spread data is the necessary next step toward any practical use.
